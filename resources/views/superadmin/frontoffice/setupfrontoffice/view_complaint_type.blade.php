        ...
        <div class="">
            <div class="page-title">
                <div class="title_left">
                    <h3>Complaint Type List</h3>
                </div>
                <div class="button-container">
                    <a href="{{ route('create.complaint.type') }}"><button type="button"
                            class="btn btn-primary btn-sm mt-1">Add Complaint Type</button></a>
                </div>
            </div>
                        ...
                        <div class="x_content">
                            <div class="table-responsive">
                                <table class="table table-striped jambo_table bulk_action" id="table">
                                    ...
                                    <tbody>
                                        @foreach ($complaint_type as $index => $complaint_ty)
                                            <tr>
                                                <td>{{ $index + 1 }}</td>
                                                <td>{{ $complaint_ty->complaint_type }}</td>
                                                <td>{{ $complaint_ty->description }}</td>
                                                <td>
                                                    <a href="{{ route('edit.complaint.type', $complaint_ty->id) }}"
                                                        class="btn btn-info btn-sm">Edit</a>

                                                    <a href="{{ route('destroy.complaint.type', $complaint_ty->id) }}"
